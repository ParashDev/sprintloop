...
function updateSprintUI() {
  const sprint = getCurrentSprint();
  const isCompleted = sprint && sprint.status === 'completed';

  const completeBtnD = document.getElementById('btn-complete-planner-sprint');
  const completeBtnM = document.getElementById('btn-complete-planner-sprint-mobile');
...
function completePlannerSprint() {
  const sprint = getCurrentSprint();
  if (!sprint || sprint.status !== 'active') return;

  const committed = sprint.plannerBacklog.filter(i => i.committed);
  const uncommitted = sprint.plannerBacklog.filter(i => !i.committed);
  const totalPts = committed.reduce((s, i) => s + (i.storyPoints || 0), 0);

  const msg = `Complete "${sprint.name}"?\n\n` +
    `Committed: ${committed.length} stories (${totalPts} pts)\n` +
    `Uncommitted: ${uncommitted.length} stories\n\n` +
